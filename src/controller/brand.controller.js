...
exports.deleteBrand = asyncHandler(async (req, res) => {
  ...
  // extract the cloudinary image name
  const parts = brand.image.split("/");
  const imageName = parts[parts.length - 1];
  const cleanedImageName = imageName.split("?")[0];

  // delete image from cloudinary
  const result = await deleteCloudinaryFile(cleanedImageName);
  if (!result) throw new customError(400, "image not deleted");

  // delete brand from DB
  const removeBrand = await brandModel.findOneAndDelete({ slug: slug });

  apiResponse.sendSuccess(res, 200, "brand deleted successfully", removeBrand);
});
